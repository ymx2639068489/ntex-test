...
(
       ...
       description       VARCHAR(100) not null comment '角色说明',
       router            VARCHAR(4000) not null comment '路由',
...
(
       ...
       rebate            VARCHAR(50) default 0 comment '返点 提成'
) comment '销售记录';
...
(
       ...
       notes             VARCHAR(300) not null comment '备注'
) comment '操作记录';
...
(
       ...
       base_product_id   VARCHAR(36) not null comment '对应基础产品id',
       create_at         DATETIME not null comment '创建时间',
       start_time        DATETIME not null comment '开始时间',
       end_time          DATETIME not null comment '结束时间',
       people_number     INT not null comment '人数 一次最多几个人',
       duration          INT not null comment '团期 团期',
